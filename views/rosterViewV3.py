...
def display():
    ...
    st.markdown("**Roster Period:**")
    st.markdown(f"`{_human_period(start_date, end_date)}`")
    st.markdown(f"**Weeks:** {len(pd.period_range(start_date, end_date, freq='W-MON')) - 1}")
    st.divider()
    ...
    # ---- Global counters / progress (before expanders) ----
    ...
    for i, ws in enumerate(week_starts):
        ...
        # First week expanded by default for convenience
        with st.expander(f"📅 **Week {i+1}:** {ws:%d %b} → {we:%d %b}"): # expanded=(i == 0) ## <-- ADD if we want first expanded
            # Quick actions row
        ...
        # 🔑 Recalculate AFTER updates so counters are instant
        shift_cols = [s for s in shifts if s in st.session_state.matrix.columns]
        total_days = len(st.session_state.matrix)
        total_shifts_selected = int(st.session_state.matrix[shift_cols].sum().sum())
        days_selected = int(st.session_state.matrix[shift_cols].any(axis=1).sum())
    ...
    st.divider()
    st.caption("💡**Tip**: Click once and counters update instantly 🚀")
    st.markdown(
        f"**Total shifts selected:** {total_shifts_selected}  \n"
